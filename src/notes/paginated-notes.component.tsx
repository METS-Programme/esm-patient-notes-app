...
const PaginatedNotes: React.FC<PaginatedNotes> = ({
  ...
  const sortDate = (myArray, order) =>
    order === "ASC"
      ? orderBy(
          myArray,
          [(obj) => new Date(obj.encounterDate).getTime()],
          ["desc"]
        )
      : orderBy(
          myArray,
  ...
  function customSortRow(
    noteA,
    noteB,
    { sortDirection, sortStates, ...props }
  ) {
    const { key } = props;
                          ...
                          {tableRows?.[i]?.encounterNote ? (
                            <div className={styles.copy}>
                              ...
                              <span className={styles.metadata}>
                                {formatTime(
                                  new Date(
                                    tableRows?.[i]?.encounterNoteRecordedAt
                                  )
                                )}{" "}
                                &middot; {tableRows?.[i]?.encounterProvider},{" "}
                            ...
                            <span className={styles.copy}>
                              {t(
                                "noVisitNoteToDisplay",
                                "No visit note to display"
                              )}
                            </span>
                          )}
                        </div>
